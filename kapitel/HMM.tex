...
<<<<<<< local
symboler som genereras då tillstånden växlar, symboler so<m motsvarar
=======
...
\begin{align*}
<<<<<<< local
b_j(k) = \textbf{P}[v_k \text{ vid tiden } t~|~q_t = S_j] ~~~~ &1 \leq j \leq N&1 \leq k \leq M.
=======
...
%\begin{equation*}
\begin{align}
\beta_t(i) = \sum_{j=1}^NA_{ij}B_j(O{t+1})\beta_{t+1}(j), ~~~~~~ &t = T-1,T-2,\dots,1 &1 \leq i \leq N.
\end{align}
%\end{equation*} 
=======
\begin{align*}
