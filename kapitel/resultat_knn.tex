...
Den största andelen korrekt klassificerade gester vi lyckades uppnå med samtliga gester tillåtna var 91.5\,\%. Antalet aktiverade
egenskaper var 10 då forward selection-algoritmen användes och 7 med backward selection, och antalet närmsta grannar i \knn-metoden var 7. Egenskaperna som
användes är listade i tabell~\ref{tab:bestfeats}. Figur~\ref{fig:knn-optimering} visar mer exakt hur andelen korrekta
...
\begin{figure}[tbp]
    \begin{center}
        ...
        \includegraphics[trim=2cm 2cm 2cm 1.8cm,width=\columnwidth,clip=true]{bilder/fwd_sel.eps}
        \label{fig:fwd_sel}}
        \subfloat[Backward selection]{
        \includegraphics[trim=2cm 2cm 2cm 1.8cm,width=\columnwidth,clip=true]{bilder/bwd_sel.eps}
        \label{fig:bwd_sel}}
    \end{center}
    \caption{Optimering av \knn-metoden med avseende på inkluderade egenskaper
och värden på $k$. Med forward selection-algoritmer fås ett minimum då $k=7$ och 10 egenskaper inkluderades. Med backward selection fås samma andel korrekta klassificeringar som minimum med $k=7$ och 7 inkluderade egenskaper.}
...
öka då värdet ökar från 7 till 13. Med valet av egenskaper och $k$ enligt forward selection presterade metoden enligt
...
\begin{table}[tbp]
	...
	värden i tabell~\ref{tab:tolkningsmatris}}}
	\smallskip
	\label{tab:prestanda}
	\begin{tabular}{c c c}
		\toprule 
...
bästa listas i tabell~\ref{tab:bestfeats}. Ett urval av sex gester visar i
figur~\ref{fig:feats1011} hur de två bästa egenskaperna tydligt samlar varje
